...
<main id="main">
  ...
  if (isset($_SESSION['loggedin']) == false) {
    ...
    $friend_stmt = $conn->prepare("SELECT * FROM friends WHERE user_id = ?");
    $friend_stmt->bind_param("i", $user_id);
    $friend_stmt->execute();
    $friend_result = $friend_stmt->get_result();

    if ($friend_result == true) {
      $friends = $friend_result->fetch_assoc();
    } else {
      echo "Unable to retrieve customer info.";
    }
  ...
  ?>
    <div class="w-full items-center justify-center">
      <form class="flex flex-col w-9/12 items-center h-auto drop-shadow-md" action="createPost.php" method="post">
    ...
    <div>
      <h1 class="text-3xl font-bold text-center mt-8">Friends</h1>
      <img class="w-12 h-12 rounded-full md:w-32 md:h-32 lg:w-24 lg:h-24 ml-4 -mt-8" src="images/<?php echo $friends['pfp'] ?>" title="Posts user image">
      <p><?php echo $friends['username'] ?></p>
    </div>
    <?php
    $cards = [];
    $stmt2 = $conn->prepare("SELECT * FROM posts p LEFT JOIN comments c ON p.post_id = c.p_post_id ORDER BY p.post_id DESC");
    $stmt2->execute();
    $result2 = $stmt2->get_result();
    while ($posts = $result2->fetch_assoc()) {
      // Check if the post already exists in the $cards array
      $found = false;
      foreach ($cards as $card) {
        if ($card['post_id'] == $posts['post_id']) {
          $found = true;
          break;
        }
      }
      ...
      if (!$found) {
        echo "<div class=\"w-9/12 justify-center items-center mt-8\" >";
  ...
  <?php
        ...
        $comments = $conn->prepare("SELECT * FROM comments WHERE p_post_id = ?");
        $comments->bind_param('i', $posts['post_id']);
        $comments->execute();
        $comment_results = $comments->get_result();

        echo "<div>";
        while ($comment = $comment_results->fetch_assoc()) {
          echo "<p class='ml-4 font-bold text-xl'>" . $comment['c_username'] . "</p>";
          echo "<p class=\"w-11/12 ml-4 mt-2 mb-2 text-lg\">" . $comment['comments'] . "</p>";
        }
        echo "</div>";
        echo "<hr class=\"mt-2\"/>";
