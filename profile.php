...
<?php
  $userId = $_POST["userID"];
  $user = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM gu_users WHERE user_id = $userId"));
...
<main id="main">
  ...
  <!--  Posts-->
  <?php
  $stmt2 = $conn->prepare("SELECT * FROM gu_posts p LEFT JOIN gu_comments c ON p.post_id = c.p_post_id WHERE p.p_user_id = $userId ORDER BY p.post_id DESC");
  $stmt2->execute();
